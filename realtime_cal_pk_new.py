...
def get_pasr_realtime_value(spark_df, cal_trend_ratio):
    """
...
def get_kdj_realtime_value(spark_df, cal_trend_ratio):
    ...
    kdj_s = 3
    w = Window.partitionBy('sec_cd').orderBy('pub_dt')
    spark_df = spark_df.withColumn('current_length', func.count('pub_dt').over(w))
    ...
    w = Window.partitionBy('sec_cd')
    spark_df = spark_df.withColumn('sec_length', func.count('pub_dt').over(w))
    spark_df = spark_df.filter(spark_df.current_length >= spark_df.sec_length * cal_trend_ratio)
...
def get_realtime_data(max_window_period, every_time_stocks, period):
    """
        从实时接口得到行情数据
    ...
    reulst_df['timestamp'] = reulst_df['timestamp'].astype('str')
...
if __name__ == "__main__":
    max_window_period = 250  # 最多拿250根bar
    ...
    period = 'day'
    reulst_df = get_realtime_data(max_window_period, every_time_stocks, period)
    # get_realtime_data(max_window_period)
    logging.info('成功从实时接口得到数据')
    spark.conf.set("spark.sql.execution.arrow.enabled", "true")
    k_line_df = spark.createDataFrame(reulst_df, schema=schema)
    ...
    # 得到修正pasr的值 及 修正的信号
    pasr_spark_df = get_pasr_realtime_value(k_line_df, cal_trend_ratio)
    ...
    # csv_output_path = 'file:///' + os.getcwd() + '/pk_data.csv'
    # all_spark_df.write.option('header', 'true').mode('overwrite').csv(csv_output_path)
